...
/// Assuming we add a spread of 6% we don't want to pay more than 6% of the
/// amount for tx fees.
const MAX_RELATIVE_TX_FEE: Decimal = dec!(0.04);
const DUST_AMOUNT: u64 = 2000; // while the technical dust limit for P2PKH is 546, and P2WPKH 294, this is set to 2000 due to high BTC transaction fees
    ...
    D: BatchDatabase,
{
    ...
    pub async fn estimate_fee_vsize(
        &self,
        vsize: usize,
        transfer_amount: bitcoin::Amount,
    ) -> Result<bitcoin::Amount> {
        ...
        estimate_fee_vsize(vsize, transfer_amount, fee_rate, min_relay_fee)
    }
}
fn estimate_fee_vsize(
    vsize: usize,
    ...
    let min_relay_fee = Decimal::from(min_relay_fee.to_sat()).mul(dec!(1.25));
