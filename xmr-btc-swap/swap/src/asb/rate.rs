...
impl Rate {
    ...
    pub fn ask(&self) -> Result<bitcoin::Amount> {
        ...
        let is_negative_spread = self.ask_spread.is_sign_negative();
        let adjusted_ask_spread = if is_negative_spread {
            self.ask_spread.mul(Decimal::NEGATIVE_ONE)
        } else {
            self.ask_spread
        };
        let additional_sats = sats * adjusted_ask_spread;
